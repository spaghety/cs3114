...
public class MemManager {
	...
	/**
	 * Constructor takes one argument memsize, creates a byte array, and creates a
	...
	public MemManager(int memsize) {
		...
		freespace = new FreeBlock[40]; // create array of linked
										// lists with the length to accomodate a terrabyte of space
		freespace[(int) (Math.log(memsize) / Math.log(2))] = new FreeBlock(0);
	}

	/**
	 * This function finds a free space and copies the data of the serialized object
	 * into it.
	 * 
	 * @param sem serialized seminar object
	 * @param id  the id of the seminar object
		...
		if (curr != null) {
			...
			while (headIndex < freespace.length) {
				...
				if (curr != null) {
					...
					FreeBlock temp = new FreeBlock(start + (int) Math.pow(2, headIndex - 1));
	...
	private void doubleSize() {
		...
		// Add a new large freeblock to fill half the new space by removing a dummy semRecord
	 ...
	 * object form.
	 * 
	 * @param key the record object referencing the location of the serialized
	 *            object
	...
	/**
	 * Removes memory from the byte array by allocating it as free space in the free
	 * block.
	 * 
	 * @param key object referencing the location of the serialized object being
	 *            removed
	...
	public boolean remove(SemRecord key) {
		...
		int freeBlockIndex = (int) Math.ceil(Math.log(key.getSize()) / Math.log(2));
	...
	/**
	 * This is a helper function to consolidate data blocks. This is recursive with the remove function
	 * @param si The size index in which you should check for neighboring blocks
	 */
	private void clean(int si) {
		int currSize = (int) Math.pow(2, si);
		FreeBlock curr = freespace[si];
		FreeBlock prev = null;
	...
	public void printFreeBlock() {
		boolean found = false;
		int maxIndex = (int)(Math.log(memory.length)/Math.log(2));
